...
f.clmm2.date <- function(df.input, ...)   
{
...
#    for(i.counter.2 in 1:cv.dependent.variables)  {
    for(i.counter.2 in i.counter.2:i.counter.2.stop)  {
      ...
      print(summary(df.temp$d.dependent))
      df.temp$d.dependent.factor <- factor(df.temp[[c.dependent]])    # clm and clmm models need to have factors for response variables  
