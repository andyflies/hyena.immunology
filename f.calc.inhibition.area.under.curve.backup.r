...
#  print(d.area.1000$value)
  ...
  d.area.sample <- sintegral(x = log2(df.input$i.sample.dilution), fx = df.input$d.percent.control.abs, n.pts = 1000, ret = FALSE)$value
  d.inhibition <- (d.area.control - d.area.sample)       # value is the estimated area under the curve
  d.prop.inhibition <- (d.inhibition / d.area.control)       # calculate the proportion of growth inhibited
