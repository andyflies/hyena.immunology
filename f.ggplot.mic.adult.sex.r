...
#          axis.ticks =        theme.blank(),
					...
					plot.margin =       unit(c(1, 20, 1, 20), "lines")      # 1 = top, 2, side, 3 = bottom, 4 = side
			), class = "options")
  }

  df.ec.data <- subset(df.master.bka, c.strain == "ec" & i.sample.number.2 == 2 & c.age == "adult" & c.id != "mp")   
  df.pm.data <- subset(df.master.bka, c.strain == "pm" & i.sample.number.2 == 2 & c.age == "adult" & c.id != "mp")   
...
#########################################################################
  ...
  df.ec.mean <- with(df.ec.mean, f.sort.frame(df.ec.mean, d.mic.mean, i.rank, c.id))
  df.pm.mean <- with(df.pm.mean, f.sort.frame(df.pm.mean, d.mic.mean, i.rank, c.id))
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  plot(lm.sex)
  hist(resid(lm.sex), main = "Escherichia coli")
  plot(d.mic.mean ~ c.sex, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.sex <- lm(d.mic.mean ~ c.sex, data = df.pm.mean)
  print(summary(lm.sex))
  plot(lm.sex)
  hist(resid(lm.sex), main = "Proteus mirablis")  
  plot(d.mic.mean ~ c.sex, data = df.pm.mean, main = "Proteus mirablis")
  ...
  print(gg.mic.means
#    + geom_point(aes(x = c.sex))
    + geom_boxplot(width = 0.4, outlier.colour = "blue", outlier.size = 3)
    + geom_point(position = position_jitter(width = 0.1))    
#    + geom_point(position = position_jitter(width = 0.1))
#    + geom_errorbar(aes(x = c.sex, 
