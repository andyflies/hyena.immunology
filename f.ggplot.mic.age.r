...
#########################################################################
  ...
  df.ec.mean <- with(df.ec.mean, f.sort.frame(df.ec.mean, date.sample.date, d.mic.mean, i.rank, c.id))
  df.pm.mean <- with(df.pm.mean, f.sort.frame(df.pm.mean, date.sample.date, d.mic.mean, i.rank, c.id))
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  lm.age <- lm(d.mic.mean ~ d.age.months, data = df.ec.mean)
  print(summary(lm.age))
  plot(lm.age)
  hist(resid(lm.age), main = "Escherichia coli")
  plot(d.mic.mean ~ d.age.months, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.age <- lm(d.mic.mean ~ d.age.months, data = df.pm.mean)
  print(summary(lm.age))
  plot(lm.age)
  hist(resid(lm.age), main = "Proteus mirablis")  
  plot(d.mic.mean ~ d.age.months, data = df.pm.mean, main = "Proteus mirablis")

  par(mfrow=c(2,3))
  lm.age.sex <- lm(d.mic.mean ~ d.age.months + c.sex, data = df.ec.mean)
  print(summary(lm.age.sex))
  plot(lm.age.sex)
  hist(resid(lm.age.sex), main = "Escherichia coli")
  plot(d.mic.mean ~ d.age.months + c.sex, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.age.sex <- lm(d.mic.mean ~ d.age.months + c.sex, data = df.pm.mean)
  print(summary(lm.age.sex))
  plot(lm.age.sex)
  hist(resid(lm.age.sex), main = "Proteus mirablis")  
  plot(d.mic.mean ~ d.age.months + c.sex, data = df.pm.mean, main = "Proteus mirablis")
  ...
  print(gg.mic.means
    ...
    + xlab("Age (months)")
    + ylab("MIC level")   
    + facet_wrap(~ c.strain + c.sex, nrow = 2, scales = "free")
    + f.theme() 
  )  
