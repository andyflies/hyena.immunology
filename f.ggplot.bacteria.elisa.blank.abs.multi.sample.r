# source("f.ggplot.bacteria.elisa.blank.abs.multi.sample.r")
                   
f.ggplot.bacteria.elisa.blank.abs.multi.sample <- function(df.input.data.frame)
{
...
#    + d.minutes.blood
  ...
  df.ec.igg.stderr <- aggregate(cbind(i.rank, d.months.to.wean, d.blank.abs) ~ c.id 
    + c.target
    + i.sample.number 
    + c.sex 
#    + d.sample.date
...
#    + d.minutes.blood
  ...
  df.pm.igg.stderr <- aggregate(cbind(i.rank, d.months.to.wean, d.blank.abs) ~ c.id 
    + c.target
    + i.sample.number 
    + c.sex 
#    + d.sample.date
...
#    size = 0.5) 
  ...
  print(gg.means
#    + geom_point()
    + geom_point(aes(x = i.rank)) # , position = "jitter")
#    + geom_jitter(aes(x = i.rank))
#    + stat_smooth(method = "lmer", se = TRUE, size = 2)
...
#      ymin = d.blank.abs - d.stderr), width = 0) 
  ...
  par(mfrow=c(3,3))
  lmer.base <- lmer(d.blank.abs ~ 1 + (1 | c.id), data = df.ec.igg, , REML = FALSE)
  lmer.rank <- lmer(d.blank.abs ~ i.rank + (1 | c.id), data = df.ec.igg, , REML = FALSE)
  lmer.age <- lmer(d.blank.abs ~ d.age.months + (1 | c.id), data = df.ec.igg, , REML = FALSE)
  lmer.rank.age <- lmer(d.blank.abs ~ i.rank + d.age.months + (1 | c.id), data = df.ec.igg, , REML = FALSE)
  lmer.rank.age.intx <- lmer(d.blank.abs ~ i.rank + d.age.months + i.rank:d.age.months + (1 | c.id), data = df.ec.igg, , REML = FALSE)
#  print(summary(lmer.rank))
  hist(resid(lmer.rank), main = "ec.igg, lmer.rank")
  hist(resid(lmer.age), main = "ec.igg, lmer.age")
  hist(resid(lmer.rank.age), main = "ec.igg, lmer.rank.age")
  hist(resid(lmer.rank.age.intx), main = "ec.igg, lmer.rank.age.intx")
  print(anova(lmer.base, lmer.rank))
  print(anova(lmer.base, lmer.age))
  print(anova(lmer.base, lmer.rank.age))
  print(anova(lmer.base, lmer.rank.age.intx)) 
  print(anova(lmer.rank, lmer.age))
  print(anova(lmer.rank, lmer.rank.age))
  print(anova(lmer.rank, lmer.rank.age.intx))

  par(mfrow=c(3,3))
  lmer.base <- lmer(d.blank.abs ~ 1 + (1 | c.id), data = df.pm.igg, , REML = FALSE)
  lmer.rank <- lmer(d.blank.abs ~ i.rank + (1 | c.id), data = df.pm.igg, , REML = FALSE)
  lmer.age <- lmer(d.blank.abs ~ d.age.months + (1 | c.id), data = df.pm.igg, , REML = FALSE)
  lmer.rank.age <- lmer(d.blank.abs ~ i.rank + d.age.months + (1 | c.id), data = df.pm.igg, , REML = FALSE)
  lmer.rank.age.intx <- lmer(d.blank.abs ~ i.rank + d.age.months + i.rank:d.age.months + (1 | c.id), data = df.pm.igg, , REML = FALSE)
#  print(summary(lmer.rank))
  hist(resid(lmer.rank), main = "pm.igg, lmer.rank")
  hist(resid(lmer.age), main = "pm.igg, lmer.age")
  hist(resid(lmer.rank.age), main = "pm.igg, lmer.rank.age")
  hist(resid(lmer.rank.age.intx), main = "pm.igg, lmer.rank.age.intx")
  print(anova(lmer.base, lmer.rank))
  print(anova(lmer.base, lmer.age))
  print(anova(lmer.base, lmer.rank.age))
  print(anova(lmer.base, lmer.rank.age.intx)) 
