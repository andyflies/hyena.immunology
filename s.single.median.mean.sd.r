# source("s.single.median.mean.sd.r")
...
df.ec.data <- subset(df.temp, c.strain == "ec" & i.sample.number == 1)   # Get only the first sample from each individual for the E. coli data
df.pm.data <- subset(df.temp, c.strain == "pm" & i.sample.number == 1)   # Get only the first sample from each individual for the P. mirabilis data

# print(df.ec.data[df.ec.data$c.id=="mrph",])

df.ec.median <- aggregate(log2(d.mic.90) ~ c.id 
  + i.sample.number 
  + c.sex 
  + d.sample.date
#  + c.mom
...
df.pm.median <- aggregate(log2(d.mic.90) ~ c.id 
  + i.sample.number 
  + c.sex 
  + d.sample.date
#  + c.mom
...
l.v.check.names <- names(df.ec.median) == "log2(d.mic.90)"
colnames(df.ec.median)[l.v.check.names]<-"d.mic.median"  
l.v.check.names <- names(df.pm.median) == "log2(d.mic.90)"
colnames(df.pm.median)[l.v.check.names]<-"d.mic.median"  

########################### aggregate and get the means #################
df.ec.mean <- aggregate(log2(d.mic.90) ~ c.id 
  + i.sample.number 
  + c.sex 
  + d.sample.date
#  + c.mom
...
df.pm.mean <- aggregate(log2(d.mic.90) ~ c.id 
  + i.sample.number 
  + c.sex 
  + d.sample.date
#  + c.mom
...
df.pm.sd <- aggregate(log2(d.mic.90) ~ c.id 
  + i.sample.number 
  + c.sex 
  + d.sample.date
#  + c.mom
...
l.v.check.names <- names(df.ec.sd) == "log2(d.mic.90)"
colnames(df.ec.sd)[l.v.check.names]<-"d.mic.sd"  
l.v.check.names <- names(df.pm.sd) == "log2(d.mic.90)"
colnames(df.pm.sd)[l.v.check.names]<-"d.mic.sd"  

#########################################################################

print(summary(df.ec.sd))
...
boxplot(d.mic.mean ~ c.age, data = df.ec.mean, 
  main = "E. coli MIC full set", xlab = "age", ylab = "log2(mic)")
boxplot(d.mic.mean ~ c.age, data = df.pm.mean, 
  main = "P. mirabilis MIC full set", xlab = "age", ylab = "log2(mic)")
