...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  lm.rank <- lm(d.mic.mean ~ i.rank + c.age, data = df.ec.mean)
  print(summary(lm.rank))
  plot(lm.rank)
  hist(resid(lm.rank), main = "Escherichia coli")
  plot(d.mic.mean ~ i.rank + c.age, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.rank <- lm(d.mic.mean ~ i.rank  + c.age, data = df.pm.mean)
  print(summary(lm.rank))
  plot(lm.rank)
  hist(resid(lm.rank), main = "Proteus mirablis")  
  plot(d.mic.mean ~ i.rank + c.age, data = df.pm.mean, main = "Proteus mirablis")
  ...
  print(gg.mic.means
    ...
    + xlab("Rank")
    + ylab("MIC level")   
    + facet_wrap(~ c.strain + c.age, nrow = 2, scales = "free")
    + f.theme() 
  )  
