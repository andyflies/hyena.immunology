...
#        lmer.alt.8, lmer.alt.9, lmer.alt.10, lmer.alt.11, lmer.alt.12, REML = FALSE)
      df.anova.output.temp <- anova(lmer.null, lmer.alt.1, lmer.alt.2, lmer.alt.3, lmer.alt.4, lmer.alt.5, lmer.alt.6, lmer.alt.7, 
        lmer.alt.8, lmer.alt.9, lmer.alt.10, REML = FALSE)
...
#      print(paste("number of columns in fixef: ", length(coef(summary(lmer.alt.1))[1,])))
  ...
  par(mfrow = c(2, 2)) 
  lmer.lactate <- lmer((d.mic.90.bka.ec) ~ i.rank +   (1 | c.id), data = df.temp.lactate, REML = FALSE)
  lmer.pregnant <- lmer((d.mic.90.bka.ec) ~ i.rank +   (1 | c.id), data = df.temp.pregnant, REML = FALSE)
#  lmer.bado <- lmer((d.mic.90.bka.ec) ~ i.rank +   (1 | c.id), data = df.temp.bado, REML = FALSE)
print("ec")
print(cftest(lmer.lactate))
print(cftest(lmer.pregnant))
  plot((d.mic.90.bka.ec) ~ i.rank, data = df.temp.lactate, main = "ec lactate")
  text((df.temp.lactate$d.mic.90.bka.ec) ~ df.temp.lactate$i.rank, labels = df.temp.lactate$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.lactate))[[1]], b = (fixef(lmer.lactate))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.mic.90.bka.ec) ~ i.rank, data = df.temp.pregnant, main = "ec prego")
  text((df.temp.pregnant$d.mic.90.bka.ec) ~ df.temp.pregnant$i.rank, labels = df.temp.pregnant$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.pregnant))[[1]], b = (fixef(lmer.pregnant))[[2]])    # a = intercept, b = slope (coefficient)
#  plot((d.mic.90.bka.ec) ~ i.rank, data = df.temp.bado, main = "ec bado")
#  text((df.temp.bado$d.mic.90.bka.ec) ~ df.temp.bado$i.rank, labels = df.temp.bado$c.id, pos = 4, cex = 1)
...
print("pm")
  lmer.lactate <- lmer((d.mic.90.bka.pm) ~ i.rank +   (1 | c.id), data = df.temp.lactate, REML = FALSE)
  lmer.pregnant <- lmer((d.mic.90.bka.pm) ~ i.rank +   (1 | c.id), data = df.temp.pregnant, REML = FALSE)
#  lmer.bado <- lmer((d.mic.90.bka.pm) ~ i.rank +   (1 | c.id), data = df.temp.bado, REML = FALSE)
print(cftest(lmer.lactate))
print(cftest(lmer.pregnant))
#print(cftest(lmer.bado))
  plot((d.mic.90.bka.pm) ~ i.rank, data = df.temp.lactate, main = "pm lactate")
  text((df.temp.lactate$d.mic.90.bka.pm) ~ df.temp.lactate$i.rank, labels = df.temp.lactate$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.lactate))[[1]], b = (fixef(lmer.lactate))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.mic.90.bka.pm) ~ i.rank, data = df.temp.pregnant, main = "pm prego")
  text((df.temp.pregnant$d.mic.90.bka.pm) ~ df.temp.pregnant$i.rank, labels = df.temp.pregnant$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.pregnant))[[1]], b = (fixef(lmer.pregnant))[[2]])    # a = intercept, b = slope (coefficient)
#  plot((d.mic.90.bka.pm) ~ i.rank, data = df.temp.bado, main = "pm bado")
#  text((df.temp.bado$d.mic.90.bka.pm) ~ df.temp.bado$i.rank, labels = df.temp.bado$c.id, pos = 4, cex = 1)
#  abline(a = (fixef(lmer.bado))[[1]], b = (fixef(lmer.bado))[[2]])    # a = intercept, b = slope (coefficient)
  hist(resid(lmer.lactate))
  hist(resid(lmer.pregnant))
#  hist(resid(lmer.bado))
...
print("done with bka")
par(mfrow = c(2, 2)) 
  lmer.lactate <- lmer((d.blank.abs.total.igg) ~ i.rank +   (1 | c.id), data = df.temp.lactate, REML = FALSE)
  lmer.pregnant <- lmer((d.blank.abs.total.igg) ~ i.rank +   (1 | c.id), data = df.temp.pregnant, REML = FALSE)
  lmer.bado <- lmer((d.blank.abs.total.igg) ~ i.rank +   (1 | c.id), data = df.temp.bado, REML = FALSE)
print("igg")
print(cftest(lmer.lactate))
print(cftest(lmer.pregnant))
print(cftest(lmer.bado))
  plot((d.blank.abs.total.igg) ~ i.rank, data = df.temp.lactate, main = "igg lactate")
  text((df.temp.lactate$d.blank.abs.total.igg) ~ df.temp.lactate$i.rank, labels = df.temp.lactate$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.lactate))[[1]], b = (fixef(lmer.lactate))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.blank.abs.total.igg) ~ i.rank, data = df.temp.pregnant, main = "igg prego")
  text((df.temp.pregnant$d.blank.abs.total.igg) ~ df.temp.pregnant$i.rank, labels = df.temp.pregnant$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.pregnant))[[1]], b = (fixef(lmer.pregnant))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.blank.abs.total.igg) ~ i.rank, data = df.temp.bado, main = "igg bado")
  text((df.temp.bado$d.blank.abs.total.igg) ~ df.temp.bado$i.rank, labels = df.temp.bado$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.bado))[[1]], b = (fixef(lmer.bado))[[2]])    # a = intercept, b = slope (coefficient)
  hist(resid(lmer.lactate))
  hist(resid(lmer.pregnant))
  hist(resid(lmer.bado))
...
print("igm")
  lmer.lactate <- lmer((d.blank.abs.total.igm) ~ i.rank +   (1 | c.id), data = df.temp.lactate, REML = FALSE)
  lmer.pregnant <- lmer((d.blank.abs.total.igm) ~ i.rank +   (1 | c.id), data = df.temp.pregnant, REML = FALSE)
  lmer.bado <- lmer((d.blank.abs.total.igm) ~ i.rank +   (1 | c.id), data = df.temp.bado, REML = FALSE)
print(cftest(lmer.lactate))
print(cftest(lmer.pregnant))
print(cftest(lmer.bado))
  plot((d.blank.abs.total.igm) ~ i.rank, data = df.temp.lactate, main = "igm lactate")
  text((df.temp.lactate$d.blank.abs.total.igm) ~ df.temp.lactate$i.rank, labels = df.temp.lactate$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.lactate))[[1]], b = (fixef(lmer.lactate))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.blank.abs.total.igm) ~ i.rank, data = df.temp.pregnant, main = "igm prego")
  text((df.temp.pregnant$d.blank.abs.total.igm) ~ df.temp.pregnant$i.rank, labels = df.temp.pregnant$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.pregnant))[[1]], b = (fixef(lmer.pregnant))[[2]])    # a = intercept, b = slope (coefficient)
  plot((d.blank.abs.total.igm) ~ i.rank, data = df.temp.bado, main = "igm bado")
  text((df.temp.bado$d.blank.abs.total.igm) ~ df.temp.bado$i.rank, labels = df.temp.bado$c.id, pos = 4, cex = 1)
  abline(a = (fixef(lmer.bado))[[1]], b = (fixef(lmer.bado))[[2]])    # a = intercept, b = slope (coefficient)
  hist(resid(lmer.lactate))
  hist(resid(lmer.pregnant))
  hist(resid(lmer.bado))
