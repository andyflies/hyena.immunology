# source("f.lm.total.ig.adult.female.rank.r")

# print(df.data[df.data$c.id=="mls", 1:10])
# df.temp <- subset(df.data, !(c.id == "mls" & c.age == "subadult"))
# print(df.temp[df.temp$c.id=="mls",])

f.lm.total.ig.adult.female.rank <- function()
{

  df.igg.data <- subset(df.data, c.isotype.target == "IgG" & c.sex == "f" & c.age == "adult")   # Get only the first sample from each individual for the E. coli data
  df.igm.data <- subset(df.data, c.isotype.target == "IgG" &  c.sex == "f" & c.age == "adult")   # Get only the first sample from each individual for the P. mirabilis data
...
#    + d.minutes.blood
  ...
  df.igm.stderr <- aggregate(cbind(d.mic.90.level, i.rank, d.months.to.wean, d.blank.abs.total.ig) ~ c.id 
    + c.isotype.target
#    + i.sample.number 
    + c.sex 
#    + d.sample.date
...
#########################################################################
  ...
  par(mfrow=c(2,3))
  lm.months.to.wean <- lm(d.blank.abs.total.ig ~ d.months.to.wean, data = df.igg.mean)
  print(summary(lm.months.to.wean))
  plot(lm.months.to.wean)
  hist(resid(lm.months.to.wean))  
  
  par(mfrow=c(2,3))
  lm.months.to.wean <- lm(d.blank.abs.total.ig ~ d.months.to.wean, data = df.igm.mean)
  print(summary(lm.months.to.wean))
  plot(lm.months.to.wean)
  hist(resid(lm.months.to.wean))  

  par(mfrow=c(2,3))
  lm.rank.months.to.wean <- lm(d.blank.abs.total.ig ~ i.rank + d.months.to.wean, data = df.igg.mean)
  print(summary(lm.rank.months.to.wean))
  plot(lm.rank.months.to.wean)
  hist(resid(lm.rank.months.to.wean))  
  
  par(mfrow=c(2,3))
  lm.rank.months.to.wean <- lm(d.blank.abs.total.ig ~ i.rank + d.months.to.wean, data = df.igm.mean)
  print(summary(lm.rank.months.to.wean))
  plot(lm.rank.months.to.wean)
  hist(resid(lm.rank.months.to.wean)) 
