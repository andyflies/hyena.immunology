# source("f.lmer.reduced.model.rank.scale.r")
...
f.lmer.reduced.model.rank.scale <- function(df.input, ...)   
{
  options(warn = 1) 
...
#  df.temp.full$d.date <- scale((df.temp.full$i.sample.months), scale = FALSE)     
#  df.temp.full$d.date <- log(df.temp.full$i.sample.months)
...
print("1")
      ...
      lmer.1 <- lmer((d.dependent) ~ (d.date) 
        + i.rank
        + (1 | c.id), 
        data = df.temp, REML = FALSE, na.action = na.omit, control = list(maxIter = 1000), verbose = TRUE) 
      l.models[1] <- lmer.1
...
#        print(l.models[[i]])
        ...
        print(cftest(l.models[[i]]))
        print("                  cftest with outliers above 4 removed ***** ***** *** ***  ")  
        l.romr.data <- romr.fnc(l.models[[i]], data = df.temp, trim = 2.5)
#        print(l.romr.data)
#        print(df.temp[,1])
        print(length(df.temp[,1]))
        print(l.romr.data$data[,1])

#        print(cbind(as.character(df.temp[,1]), as.character(l.romr.data$data[,1])))
#        print(length(l.romr.data$data[,1]))
        l.romr.models[i] <- update(l.models[[i]], data = l.romr.data$data)        
        print(l.romr.models[i])
        print("cftest(l.romr.models[[i]]")
        print(cftest(l.romr.models[[i]]))
        
        lmer.samp = mcmcsamp(l.models[[i]], n = 100)
        lmer.samp = mcmcsamp(l.models[[i]], n = 100)
        lmer.romr.samp = mcmcsamp(l.romr.models[[i]], n = 100)
        lmer.romr.samp = mcmcsamp(l.romr.models[[i]], n = 100)
        print("HPDinterval(lmer.samp, prob = 0.95)") 
        print(HPDinterval(lmer.samp, prob = 0.95)) 
        print("HPDinterval(lmer.romr.samp, prob = 0.95)") 
        print(HPDinterval(lmer.romr.samp, prob = 0.95)) 
#        mcposthoc.fnc(lmer.samp, two.tailed = TRUE, mcmc = TRUE, nsim = 100, ndigits = 8)  # var, 
#        mcposthoc.fnc(lmer.romr.samp, two.tailed = TRUE, mcmc = TRUE, nsim = 100, ndigits = 8)  # var, 
