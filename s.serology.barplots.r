...
c.target.names <- c("Canine adenovirus", "Canine coronavirus", "Canine distemper virus", "Feline immunodeficiency virus", "Filarial nematodes")
...
#    print(df.counts)
  ...
  df.counts <- df.counts[!is.na(df.counts$status), ] 
#  print(df.counts)
  df.counts$status <- factor(df.counts$status, levels = c("Negative", "Low positive", "High positive"))


  f.theme <- function(base_size = 12) {
    structure(list(
					axis.line =         theme_blank(),
					axis.text.x =       theme_text(size = base_size * 1.5, lineheight = 0.9, colour = "black", hjust = 0.5, vjust = 1, angle = 0, face = "bold"),
					axis.text.y =       theme_text(size = base_size * 1.4, lineheight = 0.9, colour = "black", hjust = 0.5, vjust = 0.5, face = "bold"),
...
#          axis.ticks =        theme.blank(),
					...
					legend.text =       theme_text(size = base_size * 1.2, face = "bold"),
