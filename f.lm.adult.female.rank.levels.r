...
#  df.ec.data <- subset(df.data, c.strain == "ec" & c.sex == "f" & c.age == "adult" & c.id != "gol")   # Get only the first sample from each individual for the E. coli data
#  df.pm.data <- subset(df.data, c.strain == "pm" & c.sex == "f" & c.age == "adult" & c.id != "gol")   # Get only the first sample from each individual for the P. mirabilis data
 

  
  ########################### aggregate and get the means #################
  df.ec.mean <- aggregate(cbind(d.mic.90.level, i.rank, d.months.to.wean) ~ c.id 
 #   + i.sample.number 
    + c.sex 
#    + d.sample.date
  #  + c.mom
...
#    + d.minutes.blood
  ...
  df.pm.mean <- aggregate(cbind(d.mic.90.level, i.rank, d.months.to.wean) ~ c.id 
#    + i.sample.number 
    + c.sex 
#    + d.sample.date
  #  + c.mom
