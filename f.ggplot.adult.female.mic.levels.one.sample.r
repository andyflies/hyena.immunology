...
#          axis.ticks =        theme.blank(),
  ...
  df.ec.mean <- aggregate(cbind(d.mic.90.level, i.rank, d.months.to.wean, i.sample.number, i.sample.number.2, d.sample.date, d.dob, d.age.months, 
    d.mass,
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  lm.glucose <- lm(d.mic.mean ~ i.glucose.red, data = df.ec.mean)
  print(summary(lm.glucose))
  plot(lm.glucose)
  hist(resid(lm.glucose), main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.glucose <- lm(d.mic.mean ~ i.glucose.red, data = df.pm.mean)
  print(summary(lm.glucose))
  plot(lm.glucose)
  hist(resid(lm.glucose), main = "Proteus mirablis")  

  par(mfrow=c(2,3))
  lm.glucose <- lm(i.glucose.red ~ i.rank, data = df.ec.mean)
  print(summary(lm.glucose))
  plot(lm.glucose)
  hist(resid(lm.glucose), main = "Escherichia coli") 
 
  par(mfrow=c(2,3))
  lm.glucose <- lm(i.glucose.red ~ i.rank, data = df.pm.mean)
  print(summary(lm.glucose))
  plot(lm.glucose)
  hist(resid(lm.glucose), main = "Proteus mirablis ") 
...
#    + scale_colour_manual(name = "Adjuvant", values = c("NO" = "grey", "YES" = "black"), legend = TRUE)
    + geom_errorbar(aes(x = d.age.months, 
      ymax = d.mic.mean + d.stderr, 
      ymin = d.mic.mean - d.stderr), width = 0) 
    + opts(title = "Bacterial killing assays") 
    + xlab("Age (months)")
    + ylab("MIC level")   
    + facet_wrap(~ c.strain, nrow = 2, scales = "free")
    + f.theme() 
  ...
  print(gg.mic.means
    + geom_point(aes(x = i.glucose.red))
    + stat_smooth(method = "lm", se = TRUE, size = 2)
#    + geom_point(aes(x = d.blank.abs, y = d.blank.abs.total.ig), size = 3.5, legend = FALSE)
#    + scale_colour_manual(name = "Adjuvant", values = c("NO" = "grey", "YES" = "black"), legend = TRUE)
    + geom_errorbar(aes(x = i.glucose.red, 
      ymax = d.mic.mean + d.stderr, 
      ymin = d.mic.mean - d.stderr), width = 0) 
    + opts(title = "Bacterial killing assays") #, color = "Andy")
    + xlab("Glucose")
...
#    + scale_colour_manual(name = "Adjuvant", values = c("NO" = "grey", "YES" = "black"), legend = TRUE)
#    + geom_errorbar(aes(x = i.glucose.red, 
#      ymax = i.rank + d.stderr, 
#      ymin = rank - d.stderr), width = 0) 
    + opts(title = "") #, color = "Andy")
    + xlab("Glucose")
    + ylab("Rank")   
    + facet_wrap(~ c.strain, nrow = 2, scales = "free")
    + f.theme() 
