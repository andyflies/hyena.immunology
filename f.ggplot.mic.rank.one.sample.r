...
#          axis.ticks =        theme.blank(),
  ...
  df.ec.data <- subset(df.master.bka, c.strain == "ec" & c.sex == "f" & i.sample.number == 1)   
  df.pm.data <- subset(df.master.bka, c.strain == "pm" & c.sex == "f" & i.sample.number == 1)   
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  lm.rank.sex <- lm(d.mic.mean ~ i.rank + c.sex, data = df.ec.mean)
  print(summary(lm.rank.sex))
#  plot(lm.rank.sex)
  hist(resid(lm.rank.sex), main = "Escherichia coli")
  plot(d.mic.mean ~ i.rank + c.sex, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.rank.sex <- lm(d.mic.mean ~ i.rank + c.sex, data = df.pm.mean)
  print(summary(lm.rank.sex))
#  plot(lm.rank.sex)
  hist(resid(lm.rank.sex), main = "Proteus mirablis")  
  plot(d.mic.mean ~ i.rank + c.sex, data = df.pm.mean, main = "Proteus mirablis")
