# source("f.lm.subadult.male.rank.levels.r")

# print(df.data[df.data$c.id=="mls", 1:10])
# df.temp <- subset(df.data, !(c.id == "mls" & c.age == "subadult"))
# print(df.temp[df.temp$c.id=="mls",])

f.lm.subadult.male.rank.levels <- function()
{

  df.ec.data <- subset(df.data, c.strain == "ec" & c.sex == "m" & c.age == "subadult")   # Get only the first sample from each individual for the E. coli data
  df.pm.data <- subset(df.data, c.strain == "pm" & c.sex == "m" & c.age == "subadult")   # Get only the first sample from each individual for the P. mirabilis data
