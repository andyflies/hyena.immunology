...
#          axis.ticks =        theme.blank(),
  ...
  df.ec.data <- subset(df.master.bka, c.strain == "ec" & i.sample.number == 1)   
  df.pm.data <- subset(df.master.bka, c.strain == "pm" & i.sample.number == 1)   
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  lm.sex <- lm(d.mic.mean ~ c.sex + c.age, data = df.ec.mean)
  print(summary(lm.sex))
#  plot(lm.sex)
  hist(resid(lm.sex), main = "Escherichia coli")
  plot(d.mic.mean ~ c.sex + c.age, data = df.ec.mean, main = "Escherichia coli")
  
  par(mfrow=c(2,3))
  lm.sex <- lm(d.mic.mean ~ c.sex  + c.age, data = df.pm.mean)
  print(summary(lm.sex))
#  plot(lm.sex)
  hist(resid(lm.sex), main = "Proteus mirablis")  
  plot(d.mic.mean ~ c.sex + c.age, data = df.pm.mean, main = "Proteus mirablis")
