...
#  print()
  df.temp.full <- subset(df.data.flat, select = c("c.id", "c.age", "c.sex", "d.sample.date", "c.reproductive.status", 
...
#    for(i.counter.2 in 1:cv.dependent.variables)  {
    for(i.counter.2 in i.counter.2:i.counter.2.stop)  {
      ...
      print(df.temp)
      df.temp <- df.temp[!is.na(df.temp$i.rank) & !is.na(df.temp$d.prey.density) & !is.na(df.temp$c.reproductive.status),]
      print(df.temp)
      
      print("")
      print(" ******* starting clmm models **************************")
print("test1")
      clmm.1 <- clmm(d.dependent.factor ~ d.independent + (1 | c.id), data = df.temp, Hess = TRUE, link = "probit", threshold = "flexible", nAGQ = 10)
print("test2")
#      clmm.2 <- update(clmm.1, ~. - d.independent)
# print(typeof(clmm.1))
# print(typeof(clmm.2))

# print(anova(clmn.1, clmm.2))
      clmm.0 <- clmm(d.dependent.factor ~ 1 + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.1 <- clmm(d.dependent.factor ~ d.independent + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.rank <- clmm(d.dependent.factor ~ i.rank + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)

      clmm2.rank <- clmm2(d.dependent.factor ~ i.rank, random = c.id, data = df.temp, Hess = TRUE, link = "logistic", threshold = "flexible", nAGQ = 10)
      clmm.repro <- clmm(d.dependent.factor ~ c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
...
print(anova(clmm2.rank,clmm2.repro))

      clmm.age <- clmm(d.dependent.factor ~ d.age.months + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.prey <- clmm(d.dependent.factor ~ d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test20")

      clmm.date.rank.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.prey.1 <- clmm(d.dependent.factor ~ d.independent + d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.repro.1 <- clmm(d.dependent.factor ~ d.independent + c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.rank.prey.1 <- clmm(d.dependent.factor ~ i.rank + d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test26")
      clmm.rank.prey.2 <- clmm(d.dependent.factor ~ i.rank * d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test27")
      clmm.rank.age.1 <- clmm(d.dependent.factor ~ i.rank + d.age.months + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test27.1")
      clmm.rank.repro.1 <- clmm(d.dependent.factor ~ i.rank + c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test28")
      clmm.rank.repro.2 <- clmm(d.dependent.factor ~ i.rank * c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29")

      clmm.date.rank.prey.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.1")
      clmm.date.rank.prey.2 <- clmm(d.dependent.factor ~ d.independent + i.rank * d.prey.density + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.2")
      clmm.date.rank.age.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + d.age.months + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.3")
      clmm.date.rank.repro.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.4")
      clmm.date.rank.repro.2 <- clmm(d.dependent.factor ~ d.independent + i.rank * c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.5")
      clmm.rank.prey.repro.1 <- clmm(d.dependent.factor ~ i.rank + d.prey.density + c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.6")
      clmm.rank.prey.repro.2 <- clmm(d.dependent.factor ~ i.rank * d.prey.density +  c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.7")
      clmm.rank.prey.repro.12 <- clmm(d.dependent.factor ~ i.rank + d.prey.density * c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)


print("test29.8")
      clmm.rank.prey.repro.22 <- clmm(d.dependent.factor ~ i.rank * d.prey.density + d.prey.density * c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.rank.age.repro.1 <- clmm(d.dependent.factor ~ i.rank + d.age.months +  c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
print("test29.9")

      clmm.date.rank.prey.repro.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + d.prey.density + c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.rank.prey.repro.2 <- clmm(d.dependent.factor ~ d.independent + i.rank * d.prey.density +  c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.rank.prey.repro.12 <- clmm(d.dependent.factor ~ d.independent + i.rank + d.prey.density * c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.rank.prey.repro.22 <- clmm(d.dependent.factor ~ d.independent + i.rank * d.prey.density + d.prey.density * c.reproductive.status +  (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
      clmm.date.rank.age.repro.1 <- clmm(d.dependent.factor ~ d.independent + i.rank + d.age.months +  c.reproductive.status + (1 | c.id), data = df.temp, Hess = TRUE, link = "logit", threshold = "flexible", nAGQ = 10)
...
print("test35")
    ...
    l.models <- list(clmm.0, lmer.0, clmm.1, lmer.1, clmm.rank, lmer.rank, clmm.repro, lmer.repro, clmm.age, lmer.age, clmm.prey, lmer.prey,
      clmm.date.rank.1, lmer.date.rank.1, clmm.date.prey.1, lmer.date.prey.1, clmm.date.repro.1, lmer.date.repro.1, 
      clmm.rank.prey.1, 
      lmer.rank.prey.1, 
      clmm.rank.prey.2,
      lmer.rank.prey.2,
      ...
      clmm.rank.repro.2, 
      lmer.rank.repro.2, 
      clmm.date.rank.prey.1, lmer.date.rank.prey.1, clmm.date.rank.prey.2, lmer.date.rank.prey.2, clmm.date.rank.age.1, lmer.date.rank.age.1, 
      clmm.date.rank.repro.1, lmer.date.rank.repro.1, clmm.date.rank.repro.2,lmer.date.rank.repro.2,
      clmm.rank.prey.repro.1, lmer.rank.prey.repro.1, clmm.rank.prey.repro.2,lmer.rank.prey.repro.2,
      clmm.rank.prey.repro.12, lmer.rank.prey.repro.12, clmm.rank.prey.repro.22, lmer.rank.prey.repro.22, 
      clmm.rank.age.repro.1, 
      lmer.rank.age.repro.1,
      clmm.date.rank.prey.repro.1, lmer.date.rank.prey.repro.1, clmm.date.rank.prey.repro.2, lmer.date.rank.prey.repro.2, 
      clmm.date.rank.prey.repro.12, lmer.date.rank.prey.repro.12, clmm.date.rank.prey.repro.22, lmer.date.rank.prey.repro.22, 
      clmm.date.rank.age.repro.1, lmer.date.rank.age.repro.1
    )
