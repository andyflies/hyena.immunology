...
Y<-rnorm(length(df.data.flat$d.mic.90.level.median.bka.pm),mean=mean(df.data.flat$d.mic.90.level.median.bka.pm),sd=sd(df.data.flat$d.mic.90.level.median.bka.pm))
...
op <- par(mfrow=c(2,2))
hist(log2(d.mic.50.bka.pm),nclass=3,main="Observed data",xlab="log2(d.mic.50.bka.pm)")
hist(log2(d.mic.50.bka.pm),nclass=5,main="Observed data",xlab="Weight",freq=F)
Y<-rnorm(length(log2(d.mic.50.bka.pm)),mean=mean(log2(d.mic.50.bka.pm)),sd=sd(log2(d.mic.50.bka.pm)))
hist(Y,nclass=5,main="Simulated data",xlab="log2(d.mic.50.bka.pm)")
X<-seq(0,30,length=200)
Y<-dnorm(X,mean=mean(log2(d.mic.50.bka.pm)),sd=sd(log2(d.mic.50.bka.pm)))
plot(X,Y,type="l",xlab="log2(d.mic.50.bka.pm)",ylab="Probablities",
