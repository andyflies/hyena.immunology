...
i.cmi.cav2.igm <- c(160, 80)  
i.cmi.cav2.igm <- c(150, 75)  

i.cmi.ccv.igg <- c(200, 100)
i.cmi.ccv.igm <- c(200, 100)
i.cmi.ccv.igm <- c(160, 80)
i.cmi.ccv.igm <- c(150, 75)

i.cmi.cdv.igg <- c(200, 100)
i.cmi.cdv.igm <- c(200, 100)
i.cmi.cdv.igm <- c(160, 80)
i.cmi.cdv.igm <- c(150, 75)

i.cmi.dirofilaria.igg <- c(200, 100)
i.cmi.dirofilaria.igm <- c(200, 100)
i.cmi.dirofilaria.igm <- c(160, 80)
i.cmi.dirofilaria.igm <- c(150, 75)

i.cmi.fiv.p24.igg <- c(200, 100)
i.cmi.fiv.p24.igm <- c(200, 100)
i.cmi.fiv.p24.igm <- c(160, 80)
i.cmi.fiv.p24.igm <- c(150, 75)
...
#					axis.ticks =        theme_blank(),
					axis.title.x =      theme_text(size = base_size * 1.4, vjust = 1, face = "bold"),
					axis.title.y =      theme_text(size = base_size * 1.4, angle = 90, vjust = 0.35, face = "bold"),
...
#					strip.label =       function(variable, value), 
					...
					plot.margin =       unit(c(1, 15, 1, 1), "lines")      # 1 = top, 2, side, 3 = bottom, 4 = side
			), class = "options")
  }

  gg.serology <- ggplot(data = subset(df.counts, c.target == "Canine adenovirus" | c.target == "Canine coronavirus" | c.target == "Canine distemper virus"), 
    aes(x = d.year,
...
#    + geom_text(label = "n = 93", size = 5, x = 0.5, y = 0.5)
  ...
  gg.serology <- ggplot(data = subset(df.counts, c.target == "Canine distemper virus" | c.target == "Feline immunodeficiency virus" | c.target == "Dirofilaria"), 
    aes(x = d.year,
      fill = factor(c.status),
      color = factor(c.status),
      legend = FALSE
