# source("f.ggplot.ig.blank.abs.multi.sample.r")
                   
f.ggplot.ig.blank.abs.multi.sample <- function(df.input.data.frame)
{
...
#  print(df.igm.temp)  
  ...
  df.igg.data <- subset(df.master.total.ig, c.isotype.target == "IgG" & c.sex == "f" & c.age == "adult")
  df.igm.data <- subset(df.master.total.ig, c.isotype.target == "IgM" &  c.sex == "f" & c.age == "adult")

  
  ########################### aggregate and get the means #################
  df.igg.mean <- aggregate(cbind(i.rank, d.months.to.wean, d.blank.abs.corrected, d.age.months) ~ c.id 
    + c.isotype.target
 #   + i.sample.number 
    + c.sex 
#    + d.sample.date
...
#    + d.minutes.blood
  ...
  df.igm.mean <- aggregate(cbind(i.rank, d.months.to.wean, d.blank.abs.corrected, d.age.months) ~ c.id 
    + c.isotype.target
#    + i.sample.number 
    + c.sex 
#    + d.sample.date
...
#    + scale_colour_manual(name = "Adjuvant", values = c("NO" = "grey", "YES" = "black"), legend = TRUE)
  ...
  lmer.base <- lmer(d.blank.abs.corrected ~  (1 | c.id), data = df.igg.data, , REML = FALSE)
  print(summary(lmer.base))
  hist(resid(lmer.base))
  
  lmer.rank <- lmer(d.blank.abs.corrected ~ i.rank + (1 | c.id), data = df.igg.data, , REML = FALSE)
  ...
  lmer.base <- lmer(d.blank.abs.corrected ~  (1 | c.id), data = df.igm.data, , REML = FALSE)
  print(summary(lmer.base))
  hist(resid(lmer.base))

  lmer.rank <- lmer(d.blank.abs.corrected ~ i.rank + (1 | c.id), data = df.igm.data, , REML = FALSE)
  print(summary(lmer.rank))
  hist(resid(lmer.rank))
  print(anova(lmer.base, lmer.rank)) 

  lmer.rank.2 <- lmer(d.blank.abs.corrected ~ i.rank + d.age.months + + i.rank:d.age.months + (1 | c.id), data = df.igm.data, , REML = FALSE)
  print(summary(lmer.rank.2))
  hist(resid(lmer.rank.2))
  print(anova(lmer.base, lmer.rank.2))
  print(anova(lmer.rank, lmer.rank.2))
    
  print("anova of only one model")
  print(anova(lmer.rank))
  print(anova(lmer.rank.2))
