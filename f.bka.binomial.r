...
#          axis.ticks =        theme.blank(),
  ...
  df.temp.full <- subset(df.bka.raw, c.sex == "f")  
#  df.temp.full <- subset(df.temp.full, c.sex == "f" & c.age == "adult")
...
#  df.temp <- subset(df.temp, !(c.id == "jj" & i.sample.number.2 == 1))
  ...
  df.igg$d.dependent <- (df.igg$d.absorbance)
  df.igm$d.dependent <- (df.igm$d.absorbance)
...
#  df.pm$d.dependent <- (df.pm$d.sintegral.mic) / 8
  ...
  df.ec$d.independent <- log(df.ec$d.age.months)
  df.pm$d.independent <- log(df.pm$d.age.months)
  df.igg$d.independent <- log(df.igg$d.age.months)
  df.igm$d.independent <- log(df.igm$d.age.months)
  ...
  lmer.1.ec <- lmer(d.dependent ~ d.independent + I(d.independent^2) + (1 | c.id), data = df.ec, REML = FALSE)
...
#      qqmath(ranef(lmer.alt, postVar = TRUE), strip = FALSE)$c.id
  ...
  lmer.1.pm <- lmer((d.dependent) ~ d.independent + I(d.independent^2) + (1 | c.id), data = df.pm, REML = FALSE)
...
#      qqmath(ranef(lmer.alt, postVar = TRUE), strip = FALSE)$c.id
  ...
  lmer.0.igg <- lmer((d.dependent) ~ 1 + (1 | c.id), data = df.igg, REML = FALSE)
  lmer.1.igg <- lmer((d.dependent) ~ d.independent + I(d.independent^2) + (1 | c.id), data = df.igg, REML = FALSE)
  ...
  hist(resid(lmer.1.igg), main = paste("igg", " ~ i.rank"))
...
#      qqmath(ranef(lmer.alt, postVar = TRUE), strip = FALSE)$c.id
  ...
  lmer.0.igm <- lmer((d.dependent) ~ 1 + (1 | c.id), data = df.igm, REML = FALSE)
  lmer.1.igm <- lmer((d.dependent) ~ d.independent + I(d.independent^2) + (1 | c.id), data = df.igm, REML = FALSE)
  ...
  hist(resid(lmer.1.igm), main = paste("igm", " ~ i.rank"))
