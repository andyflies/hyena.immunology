...
f.glmer.poisson.rank <- function(df.input, ...)   
{
...
#  Remove MP sample 1 to see how this affects the results.  It is a possible outlier
  ...
  df.temp.full <- subset(df.temp.full, !(c.reproductive.status == "neither" | c.reproductive.status == "bado"))
...
#  c.dependent <- "d.mic.90.level.bka.ec"
  ...
  cv.dependent.variables <- c("d.mic.90.level.median.bka.ec", "d.mic.90.level.median.bka.pm") # , "d.mic.90.bka.pm", "d.percent.inhibition.ec", "d.percent.inhibition.pm", "d.blank.abs.total.igg",	"d.blank.abs.total.igm") 
  ...
  for(i.counter.1 in i.counter.1:i.counter.1.stop)  {
    ...
    for(i.counter.2 in i.counter.2:i.counter.2.stop)  {
      ...
      print("mean(df.temp[[c.dependent]])")
      print(mean(df.temp[[c.dependent]]))
#      df.temp[[c.dependent]] <- df.temp[[c.dependent]] > mean(df.temp[[c.dependent]]) 
      print("median(df.temp[[c.dependent]])")
      print(median(df.temp[[c.dependent]]))
#      df.temp[[c.dependent]] <- df.temp[[c.dependent]] > median(df.temp[[c.dependent]]) 
...
#      }
      ...
      print(" ********** lm.1 <- lm(d.dependent ~ d.independent, data = df.temp, family = poisson)***********")
      glm.1 <- glm(d.dependent ~ d.independent, data = df.temp, family = poisson)
      print("summary(glm.1)")
      print(summary(glm.1))
      plot(glm.1)
...
# mcmc = pvals.fnc(primingHeid.lmer, nsim=10000, withMCMC=TRUE)
      ...
      glmer.0 <- glmer(d.dependent ~ 1 + (1 | c.id), data = df.temp, family = poisson, REML = FALSE)
      glmer.1 <- glmer(d.dependent ~ d.independent + (1 | c.id), data = df.temp, family = poisson, REML = FALSE)
