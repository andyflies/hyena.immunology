...
#  c.data.set.name <- "full.set"
  c.data.set.name <- "adult.female.reduced.as.needed"  # Inside the loop we use !is.na to remove any records that have na values
...
#    if(c.dependent == "i.cubs.weaned" | c.dependent ==  "d.repro.lifespan" | c.dependent == "d.ars"
#      | c.dependent == "d.cub.surv" | c.dependent == "d.littersize" | c.dependent == "d.litters.per.year")  {
        ########################### aggregate and get the means #################
#      df.temp <- aggregate(cbind(i.rank, 
#        d.mic.90.level.bka.ec, d.mic.90.level.bka.pm, d.blank.abs.total.igg, d.blank.abs.total.igm, 
#        d.blank.abs.ec.igg, d.blank.abs.ec.igm, d.blank.abs.pm.igg, d.blank.abs.pm.igm,
...
#        "d.blank.abs.total.igm", "d.blank.abs.ec.igg", "d.blank.abs.ec.igm", "d.blank.abs.pm.igg", "d.blank.abs.pm.igm" 
      df.temp <- aggregate(cbind(df.temp[[c.dependent]], df.temp[[c.independent]] 
#        "i.glucose.green", "i.glucose.red", "d.pcv", "d.total.solids",
#        "i.cmi.bsa.igg", "i.cmi.bsa.igm", "i.cmi.cav2.igg", "i.cmi.cav2.igm", "i.cmi.ccv.igg", "i.cmi.ccv.igm", "i.cmi.cdv.igg", "i.cmi.cdv.igm", 
#        "i.cmi.dirofilaria.igg", "i.cmi.dirofilaria.igm", "i.cmi.fiv.p24.igg", "i.cmi.fiv.p24.igm", "d.serology.index.igg", 
#        "d.serology.index.rank.igg", "d.serology.index.igm", "d.serology.index.rank.igm", "d.eosinophils", "d.lymphocytes", 
...
print(df.temp)
      colnames(df.temp) <- c("c.id", c.dependent, c.independent)
print(df.temp)
        print("After aggregating fitness measures")
        i.subset.c.id <- length(df.temp$c.id)
        i.subset.unique.c.id <- length(unique(df.temp$c.id))
        i.subset.i.rank <- length(df.temp$i.rank)
        i.subset.unique.i.rank <- length(unique(df.temp$i.rank))
...
#    print("vcov(lm.alt.1)@factors$Cholesky")
#    print(vcov(lm.alt.1)@factors$Cholesky)
    ...
    # Computing a pval for the summary lm.alt.1 model -> results are the same as cftest.  cftest reports at z value and z test p value because we are using a normal distribution to obtain the p value
#    tval <- attributes(summary(lm.alt.1))$coef[, 3]
#    pval <- 2 * pnorm(abs(tval), lower = FALSE)
#    print(cbind(attributes(summary(lm.alt.1))$coef, `p value` = signif(pval, 5)))

    print("cftest(lm.alt.1)")
    print(cftest(lm.alt.1))     # computes p-vals for the lm

  #      print(confint(cftest(lm.alt.1)))     # computes p-vals for the lm
    # ANOVA of models
