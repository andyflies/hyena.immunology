# source("f.lmer.reduced.model.rank.proportion.r")
...
f.lmer.reduced.model.rank.proportion <- function(df.input, ...)   
{
...
#  df.temp.full <- subset(df.temp.full, c.sex == "f" & i.sample.number.2 == 2)
  ...
  df.temp.full <- subset(df.temp.full, !is.na(d.rank.proportion))  
...
#          + d.cortisol
          ...
          + (1 | c.id), 
          data = df.temp, REML = FALSE) 
        print(length(resid(lmer.18)))
        print(cftest(lmer.18))
        plot(jitter(resid(lmer.18)) ~ jitter(fitted(lmer.18)), main = terms(lmer.18))
        hist(resid(lmer.18)) #, breaks = sort(unique(lmer.18$d.residuals))))
        qqnorm(resid(lmer.18))
        qqline(resid(lmer.18))
        d.resid.mean <- mean(resid(lmer.18))
        d.resid.var <- var(resid(lmer.18))
        print(paste("resid.mean, var: ", d.resid.mean, d.resid.var))
        print(lillie.test(resid(lmer.18)))
        print(ad.test(resid(lmer.18)))
        print(cvm.test(resid(lmer.18)))

        lmer.19 <- lmer((d.dependent) ~ (d.date) 
          + d.precipitation # * d.prey.density
          + (1 | c.id), 
          data = df.temp, REML = FALSE) 
        print(length(resid(lmer.19)))
        print(cftest(lmer.19))
        plot(jitter(resid(lmer.19)) ~ jitter(fitted(lmer.19)), main = terms(lmer.19))
        hist(resid(lmer.19)) #, breaks = sort(unique(lmer.19$d.residuals))))
        qqnorm(resid(lmer.19))
        qqline(resid(lmer.19))
        d.resid.mean <- mean(resid(lmer.19))
        d.resid.var <- var(resid(lmer.19))
        print(paste("resid.mean, var: ", d.resid.mean, d.resid.var))
        print(lillie.test(resid(lmer.19)))
        print(ad.test(resid(lmer.19)))
        print(cvm.test(resid(lmer.19)))

        lmer.20 <- lmer((d.cortisol) ~ (d.date) 
          + d.rank.proportion # * d.prey.density
          + (1 | c.id), 
          data = df.temp, REML = FALSE) 
        print(length(resid(lmer.20)))
        print(cftest(lmer.20))
        plot(jitter(resid(lmer.20)) ~ jitter(fitted(lmer.20)), main = terms(lmer.20))
        hist(resid(lmer.20)) #, breaks = sort(unique(lmer.20$d.residuals))))
        qqnorm(resid(lmer.20))
        qqline(resid(lmer.20))
        d.resid.mean <- mean(resid(lmer.20))
        d.resid.var <- var(resid(lmer.20))
        print(paste("resid.mean, var: ", d.resid.mean, d.resid.var))
        print(lillie.test(resid(lmer.20)))
        print(ad.test(resid(lmer.20)))
        print(cvm.test(resid(lmer.20)))
...
#  df.temp.full$d.pm.igm <- df.temp.full$d.blank.abs.pm.igm
      ...
      print(aictab(cand.set = list(lmer.1, lmer.2, lmer.3, lmer.4, lmer.5, lmer.6, lmer.7, lmer.8, lmer.9, lmer.10, lmer.11, 
        lmer.12, lmer.13, lmer.14, lmer.15, lmer.16, lmer.17, lmer.18, lmer.19), 
        modnames = c("lmer.1", "lmer.2", "lmer.3", "lmer.4", "lmer.5", "lmer.6", "lmer.7", "lmer.8", "lmer.9", "lmer.10", 
        "lmer.11", "lmer.12", "lmer.13", "lmer.14", "lmer.15", "lmer.16", "lmer.17", "lmer.18", "lmer.19"), 
        sort = TRUE))
      }
