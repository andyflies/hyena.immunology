...
f.lmer.date.2 <- function(df.input, ...)   
{
...
#  df.temp.full <- subset(df.temp.full, c.age == "adult")
...
#  df.temp.full <- subset(df.temp.full, d.prey.density > 0)
...
#  print(df.temp.full$d.prey.density)
...
#        print(cvm.test(resid(lm.rank.1)))
        ...
        df.temp$d.dependent <- (df.temp$d.dependent)
        df.temp$d.independent <- (df.temp$i.sample.years)

        lmer.1 <- lmer(log2(d.dependent) ~ d.date + ((d.precipitation)) * d.prey.density + c.sex * ((d.prey.density)) + (1 | c.id), 
          data = df.temp, REML = FALSE) 
        print(summary(lmer.1))
        print(cftest(lmer.1))
        plot(jitter(d.dependent) ~ d.date, data = df.temp, main = terms(lmer.1))
