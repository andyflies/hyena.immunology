# source("f.kw.adult.female.rank.levels.r")

# Kruskal-Wallis test for the effect of rank on MIC
#  KW does not handle ties well

# print(df.data[df.data$c.id=="mls", 1:10])
# df.temp <- subset(df.data, !(c.id == "mls" & c.age == "subadult"))
# print(df.temp[df.temp$c.id=="mls",])

f.kw.adult.female.rank.levels <- function()
{
...
#  print(paste("correlation between minutes from blood for E. coli and P. mirabilis: ", m.bacteria.minutes.blood.correlation), quote = FALSE)
  ...
  par(mfrow=c(2,3))
  kw.rank <- kruskal.test(d.mic.mean ~ i.rank, data = df.ec.mean)
  print(summary(kw.rank))
  print(kw.rank)
#  plot(kw.rank)
#  hist(resid(kw.rank))
  
  par(mfrow=c(2,3))
  kw.rank <- kruskal.test(d.mic.mean ~ i.rank, data = df.pm.mean)
  print(summary(kw.rank))
  print(kw.rank)
#  plot(kw.rank)
#  hist(resid(kw.rank))  
